...
my (@elem_order);
my (@lines);
my (%number2color, %mol2value, $coloring);
...
sub Color {
  ...
  if (defined $coloring) {
    if (defined $mol2value{$elem}) {
      $v = $mol2value{$elem};
    } elsif (defined $attr_values{$elem}{locusid}) {
      my $ll = $attr_values{$elem}{locusid};
      if (defined $mol2value{$ll}) {
        $v = $mol2value{$ll};
      }
    }
  }
  if (defined $v) {
    my $color = $coloring->Value2Color($v);
    if ($color =~ /^[0-9a-fA-F]+$/) {
      return "#" . $color;
    } else {
      return $color;
    }
  } elsif (defined $attr_values{$elem}{color}) {
...
sub DoValue {
  ...
  if ($value !~ /^(-|\+)?\d+(\.\d*)?$/ && $value !~ /^(-|\+)?\.\d+$/) {
    print STDERR "number expected for $value as expression value";
    return;
  } else {
    $mol2value{$mol} = $value;
  }
...
sub DoColor {
  ...
  if ($number !~ /^(-|\+)?\d+(\.\d*)?$/ && $number !~ /^(-|\+)?\.\d+$/) {
    print STDERR "number expected for $number in color command";
    return;
  }
#  if ($color !~ /^#?([0-9A-Fa-f]{6})/) {
#    AddError("six-place hexadecimal color specification " .
#        "expected for $color");
#    return;
#  } else {
#     $color = $1;
...
sub ParseExpression {
  ...
  while (<$exprf>) {
    ...
    if ($line =~ /^color\s+(.+)/) {
      my ($number, $color) = split(/\s+/, $1);
      DoColor($number, $color);
    } else {
      my ($mol, $value) = split(/\s+/, $line);
      DoValue($mol, $value);
    }
  }
  $coloring = new Coloring(MakeColorScale(\%number2color));
}
...
#    return;
  ...
  if ($type eq "complex") {
    ...
    for my $a ("shape") {
      if (defined $attr_values{$elem}{$a}) {
        push @attrs, "$a=\"$attr_values{$elem}{$a}\"";
      } elsif (defined $style{node}{$a}{$type}) {
