...
######################################################################
sub DoChannel {
  my ($self, $channel, $molecule, $is_mol_id) = @_;
  ...
  if ($is_mol_id eq "mol_id") {
    $molecule = Trim($molecule);
    if ($molecule =~ /^\d+$/) {
      $self->{mol_id_channel}{$molecule}{$channel} = 1;
    } else {
      $self->AddError("Illegal mol_id for channel  command: " .
          join("\t", $channel, $molecule, $is_mol_id));
    }
  } else {
...
sub CheckMol {
  ...
  if (defined $self->{mol_name}) {
    print STDOUT "In CheckMolName call\n";
    my @temp = keys %{ $self->{mol_name} };
    my $scan = 0;
    # for (my $i = 0; $i <= $#temp; $i++) {
      # my $scancheck = Scan($temp[$i]);
      # $scan = $scan + $scancheck;
      # print STDOUT "After Scan:  $temp[$i] $scancheck\n"; 
    #}
    if ($scan == 0) {
...
##!!!!!!! We aren't going to require mol type any longer
  ...
  my ($alias, $alias_list, $symbol, $name_type);
...
print STDERR "Sql:  $sql\n";
    ...
    for my $id (@temp) {
      ...
      # if(!$stm->execute()) { 
        ...
        print STDERR "In error\n"; 
        return;
      }
    print STDERR "Before while\n"; 
    while (($molid, $molname) =
        $stm->fetchrow_array()) {
      print STDERR "In while:  $molid $molname\n";
      $self->{molmap}{$molname}{$molid} = 1;
      # if (defined $self->{mol_name}{$molname}) {
        print STDERR "In defined\n";
        for $what (keys %{ $self->{mol_name}{$tempMolName} }) {
          print STDERR "In for $tempMolName\n"; 
          if ($what eq "prune") {
...
sub CheckParams {
  ...
  # $molcheck = Scan($molecule);
  
  print STDERR "After molcheck:  $molecule $molcheck\n";
  ##
