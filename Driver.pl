...
## Set a timer
##
my $query_timeout = 600;       ## max length (in seconds) for a request
...
sub ReadInput {
  ...
  if ($input_type eq "xml") {
    ...
    $mm->MatchMols();
    $pw->RemapMols();
  } elsif ($input_type eq "reactome") {
    use ParseReactomeBioPAX;
    $pw = new Pathway($lv);
    my $reactome = new ParseReactomeBioPAX($lv, $pw, $idm);
    my $fh = new FileHandle;
    open($fh, $f) or die "cannot open $f";
    $reactome->parse($fh);
    $pw->MergeMolecules();
...
sub WriteOutput {
  ...
  if ($output_type eq "html") {
    ...
    ## split up the sub-pathways in reactome
    if ($split) {

      use XMLOutput;
      for my $pid (@{ $pw->PathwayId() }) {
        my $pexid = $pw->PathwayExId($pid);
        print STDERR "#xml_file\t$pexid.xml\n";
        open(OUTF, ">$pexid.xml") or die "cannot open $output_file";
        my $copy = new Pathway($lv);
        $pw->CopyPathway($copy, $pid);
        my $xml = new XMLOutput($copy, $lv, *OUTF);
        $xml->PrXML($pid);
        close OUTF;
      }
  ...
  } elsif ($output_type eq "equation") {

    use EquationOutput;
    open(OUTF, ">$output_file") or die "cannot open $output_file";
    my $eq = new EquationOutput($pw, $lv, *OUTF);
    print OUTF "<html>\n";
    $eq->EquationOutput();
    close OUTF;
  ...
  } elsif ($output_type eq "rule") {

    use RapSheet;
    open(OUTF, ">$output_file") or die "cannot open $output_file";
    my $rule = new RapSheet($pw, $lv);
    $rule->RapAll();
    my $re = $rule->RapErrors();
    for my $atom (keys %{ $re }) {
      my @pways;
      for my $pid (@{ $pw->AtomPathway($atom) }) {
        push @pways, $pw->PathwayName($pid);
      }
      for my $err (keys %{ $$re{$atom} }) {
        print OUTF "$atom\t$err\t" . join(", ", @pways) . "\n";
      }
    }
    close OUTF;
...
sub ReadOptions {
  ...
  if (@ARGV == 0) {
    print join("\n\t",
      "options:",
      "it: input type (xml, js, reactome)",
      "ot: output type (xml, biopax, dot, table, sql, rule)",
      "if: input file",
      "of: output file",
      ...
      "split: for xml output, break up into separate pathways",
    ) . "\n";
    exit;
  }
