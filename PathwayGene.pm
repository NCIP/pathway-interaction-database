...
sub GetPathwayHitsForEntrezGenes {
  my ($db, $schema, $genes, $source, $pd, $gd) = @_;
...
where
  ...
  and p.pathway_source_id = $source
...
sub RankPathwaysByGeneHits {
  my ($db, $schema, $format, $source, $terms_a, $terms_b) = @_;
  ...
  print STDERR "In RankPathways $schema $format $source\n";
  my @terms = keys %terms;
  if (@terms == 0) {
    return;
  }
  ...
  $source_name = 'BioCarta data';
  if ($source == 5) {
    $source_name = 'NCI-Nature Curated data';
  }
  if ($source == 7) {
    $source_name = 'Reactome data';
  }
  if ($source_name eq 'BioCarta data') {
    $source = 2;
  }
  ...
  print STDERR "After GetTotalGenesInPathways\n";
  my ($genes_a, $genes_b) = GetEntrezGeneIds($db, $schema, $terms_a, $terms_b, $source, \%gd);
  my $genelist=''; 
  for my $gtest (keys % {$genes_a}) {
    $genelist = $genelist . ' ' .$gtest; 
  }
  print STDERR "Before GetPathwayHitsForEntrezGenes $genelist\n"; 
  GetPathwayHitsForEntrezGenes($db, $schema, $genes_a, $source, \%pd, \%gd);
  GetPathwayHitsForEntrezGenes($db, $schema, $genes_b, $source, \%pd, \%gd);
...
## make symbol lists
    ...
    if ($format eq "text") {
      ...
      push @{ $tmp{$pval} }, join("\t",
        "<td><a href=\"/search/pathway_landing.shtml?" .
          "what=graphic\&" .
          "pathway_id=$pid\&" .
          "jpg=on\&" .
  ...
  #  }
  #}
  print STDERR "Before assigning rows\n";
  if ($format eq "html") {
    
    unshift @rows, "<h1 class=\"pagetitle\">Batch query results for $source_name </h1> " .
        "<table class=\"data\" >" .
        "<thead> ".
        "<th scope=\"colgroup\" class=\"colspan\">Pathway Name</th><th scope=\"colgroup\" class=\"colspan\">Biomolecules in Group 1</th><th scope=\"colgroup\" class=\"colspan\">Biomolecules in Group 2</th><th scope=\"colgroup\" class=\"colspan\">P-value <div class=\"helpbox\"> " . 
        "<a href=\"/index.shtml#\" class=\"help\">Help</a> " .
        "<div class=\"description\"> " .
  	"<p class=\"norm\">The pathways are ranked by the probability that they include biomolecules from the query list.  The lower the p-value the greater the probability that the query list is biased towards a given pathway. The parameters for generating the p-value are the size of the query set, the number of biomolecules in a given pathway and the number of molecules in the database as a whole.</p></div></div></th></tr> "; 
    push @rows, "</thead>"; 
    push @rows, "<tbody>";
    for my $pval (sort numerically keys %tmp) {
      for my $r (@{ $tmp{$pval} }) {
         push @rows, ($n %2 == 0) ? "<tr>" : "<tr class=\"odd\">";
         $n++;
         push @rows, $r;
      }
    }
...
sub GetEntrezGeneIds {
  my ($db, $schema, $terms_a, $terms_b, $source, $gd) = @_;
...
where
  ...
  and e.organism = 'Hs'
  and pg.pathway_id = p.pathway_id
  and p.pathway_source_id = $source
  and e.ll_id in ($list)
!;
    my $stm = $db->prepare($sql);
  ...
  my $list;
  my $sqlsearch = "in";
  if ($#non_numeric_terms < 1) {
   $sqlsearch = "like";
  } 
  my $number = $#non_numeric_terms;
  print STDERR "Nonnumeric:  $number\n"; 
...
where
  ...
  and p.pathway_source_id = $source
  and e.organism = 'Hs'
  and ms.mol_id = ms2.mol_id
  and lower(e.symbol) = lower(ms2.map_name)
  and lower(ms.map_name) $sqlsearch ($list) 
!;
    ...
    my ($entrez_gene_id, $lc_symbol, $symbol, $symbol_type, $map_name);
    while (($entrez_gene_id, $lc_symbol, $symbol, $symbol_type, $map_name) =
        $stm->fetchrow_array()) {
      $gd->{symbol}{$entrez_gene_id}{$symbol_type} = $symbol;
      my $lengtha = length($list);

      my $teststring = substr($list,$lengtha-2,1);
      print STDERR "Lengthtest:  $teststring $lengtha $list\n";
      if ((substr($list,$lengtha-2,1) eq '%') || (defined $$terms_a{$map_name})) {
        $genes_a{$entrez_gene_id} = 1;
      }
      if (defined $$terms_b{$map_name}) {
        $genes_b{$entrez_gene_id} = 1;
      }
...
!;
  ...
  if ($format ne 'svg') {
     $inf = PREDEF_DIR . "/" . "$source_map{$source}/$pid.dotg";
  }
  ...
  if ($format eq "svg") {
    push @tmp, 
        "<embed type=\"image/svg-xml\" height=800 width=1000 " .
         "src=\"/pidcgi/GetPwImage.pl?" .
         "GID=$cache_id&FORMAT=SVG\" alt = \"Image of Pathway\">";
  } elsif ($format eq "jpg") {
    push @tmp, "<img " .
        "src=\"/pidcgi/GetPwImage.pl?" .
        "GID=$cache_id&FORMAT=JPG\" usemap=#map_$cache_id alt = \"Image of Network Search Map\"  >";
...
sub GetMolIdsForEntrezGene {
  ...
  print STDERR "In GetMolIdsForEntrezGene\n";
  my @genearray = keys %{ $genes };
  my $gene_number = $#genearray;
  print STDERR "Gene 0:  $genearray[0]\n";
  print STDERR "Gene 1:  $genearray[1]\n";
  print STDERR "Genes:  $gene_number\n";
  if ($#genearray == -1) {
    print STDERR "Empty genes\n"; 
    return {};
  }

  my ($mol, %mols, $list);

  for(my $i = 0; $i <= $#genearray; $i += ORACLE_LIST_LIMIT) {
 
    if(($i + ORACLE_LIST_LIMIT - 1) <= $gene_number) {
      $list = join("','", $genearray[$i..$i+ORACLE_LIST_LIMIT-1]);
    }
    else {
      for (my $j = 0; $j <= $#genearray; $j++) {
        $list = $list . "'" . $genearray[$j] . "'";
        if ($j < $#genearray) {
          $list = $list . ",";
        }
      }
      # $list = join("','", $genearray[$i..$gene_number]);
    }
    # $list = "'" . $list . "'";
